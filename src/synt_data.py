...
class SyntDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        for i,  freqs in enumerate(class_desc):
            try:
                with open( f"output/data/synt_class={i}.npy", "rb") as f:
                    temp = np.load(f)
                    time_series, masks, labels = temp["signal"], temp["mask"], temp["label"] 
                ...
                with open( f"output/data/synt_class={i}.npy", "wb") as f:
                    np.savez( f, signal = time_series.numpy(), mask = masks.numpy(), label = labels.numpy() )
        ...
        for i in range(self.n_class + 1):
            _, P = F.welch( self.data[self.labels == i].numpy(), fs = fs, nperseg = fs*2, axis = 1 )
    ...
    def __getitem__(self, idx, return_mask = False):
        if return_mask:
    ...
    if desc is not None and len(desc) > 0:
        for descriptor in desc:
            ...
            if "sec" in descriptor:
                ...
                dist_len = np.random.randint(int( start * length ), int( end * length  )  ) - int( start * length )
                dist_start = np.random.randint( start * length, (end * length - dist_len ) )
            else:
            ...
            if dist_start != 0:
                ...
                filt[dist_start - start_spacing: dist_start + (3 * sigma)] = gaussian_filter1d(filt[dist_start - start_spacing: dist_start + (3 * sigma)], sigma)

            if dist_start + dist_len != n_samples:
                end_spacing = 3 * sigma if n_samples - dist_start + dist_len > 3 * sigma else n_samples - dist_start + dist_len
                filt[dist_start + dist_len - (3 * sigma): dist_start + dist_len + end_spacing] = gaussian_filter1d(filt[dist_start + dist_len - (3 * sigma): dist_start + dist_len + end_spacing], sigma)

            filtered_signal += amp * filt
                         
    return torch.tensor(filtered_signal, dtype=torch.float32), torch.tensor(_mask, dtype=torch.long)
