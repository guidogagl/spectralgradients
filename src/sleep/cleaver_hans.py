...
def plot_bands(
    ...
    output_dir = f"output/model/{net}/{dataset}/plot/nrem3/cleaverhans/{index}/"
...
def plot_spectral_gradients(
    ...
    labels = [ "Wake", "NREM1", "nrem3", "NREM3", "REM" ]
...
if __name__ == "__main__":
    ...
    # we want to identify cleaver hans behavior in the model
    # so we need to select the epochs for which the delta band is NOT the most important band
    
    #sigma = np.argmax( nrem3_w, axis = -1 ) == 2
    notdelta = np.argmax( nrem3_w, axis = -1 ) != 5
    
    if len(notdelta) == 0:
        print("No epochs found with sigma band as most important band")
        exit()
    else:
        nrem3 = nrem3[notdelta]
        nrem3_w = nrem3_w[notdelta]
        nrem3_preds = nrem3_preds[notdelta]
        true_positive = true_positive[notdelta]  # Update true_positive to match filtered epochs
        indexes = indexes[notdelta]  
        
        print("Number of epochs with not delta band as most important band: ", nrem3.shape[0])
