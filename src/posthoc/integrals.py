...
def step_integral(
    ...
    if jac_path is None:
        Jp0 = torch.func.vmap(torch.func.jacfwd(path))( torch.ones(1, device=device) * t[i - 1]).squeeze() 
        Jp1 = torch.func.vmap(torch.func.jacfwd(path))( torch.ones(1, device=device) * t[i] ).squeeze()
    else:
        Jp0 = jac_path( t[i - 1] )
        Jp1 = jac_path( t[i] )
...
def path_integral(
    ...
    for t in T:
        ...
        if jac_path is None:
            J = torch.func.vmap(torch.func.jacfwd(path))( torch.ones(1, device=p.device) * t).squeeze() 
        else:
            J = jac_path( t )
