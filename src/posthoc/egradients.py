...
def expected_line_integral(fn: nn.Module, x: torch.Tensor, baselines: torch.Tensor):
...
def expected_Lineh_integral(
    ...
    T = torch.linspace(0, 1, baselines.shape[0], device=x.device, dtype=x.dtype)
...
class ExpectedGradients(nn.Module):
    def __init__(
        ...
        self.baselines = torch.stack(self.baselines, dim=0).reshape(
            n_points, len(baselines), *baselines.shape[1:]
        )
