...
class SpectralGradients(nn.Module):
    def __init__(self, 
            ...
            line_func : callable = segment_fun,
            n_points : int = 2,
    ):
        super().__init__()
        if noverlap is None:
        ...
        freqs, _ = signal.welch(torch.zeros(nperseg), fs, nperseg=nperseg, noverlap=noverlap)
        ...
        self.f = torch.cat([self.f[:-1].unsqueeze(0), self.f[1:].unsqueeze(0)], dim=0).transpose(0, 1)
    ...
    def forward(self, x):
        ...
        for filter in tqdm(self.filters):
            
            x_i1 = filter(x)
            
            grads = trapezoid_rule( self.fun, x_i, x_i1, self.line_func, self.n_points )
    ...
    def plot_power_spectral_density(self, x):
        ...
        for i, filter in enumerate( self.filters):
            if i % 20 == 0:
                filt_x = filter(x)
                f, Pxx = signal.welch(filt_x.view(-1).numpy(), fs, nperseg=nperseg, noverlap=noverlap)
...
class BandStopFilterFun(Function):
    ...
    def forward(ctx, input, a, b):
        ...
        result = signal.filtfilt(b.cpu().numpy(), a.cpu().numpy(), input.cpu().numpy().astype(np.float64) ).copy()
